...
(defn parse [s]
  (let [r #"(toggle|turn on|turn off) (\d+),(\d+) through (\d+),(\d+)"
        m (re-find r s)]
    (cons (get {"toggle" :toggle
                "turn on" :turn-on
                "turn off" :turn-off}
               (m 1))
          (map read-string (drop 2 m)))))

(defn applicable-steps [steps x y]
  (filter (fn [[_ x1 y1 x2 y2]]
            (and (<= x1 x x2)
                 (<= y1 y y2))) steps))
...
(defn strategy-1
  "Should give 543903"
  [actions]
  (loop [v 0 actions actions]
    (if (empty? actions)
      v
      (condp = (first actions)
        :turn-on (recur 1 (rest actions))
        :turn-off (recur 0 (rest actions))
        :toggle (recur (- 1 v) (rest actions))))))


(defn strategy-2
  "Should give 14687245"
  [actions]
  (loop [v 0 actions actions]
    (if (empty? actions)
      v
      (condp = (first actions)
        :turn-on (recur (inc v) (rest actions))
        :turn-off (recur (max 0 (dec v)) (rest actions))
        :toggle (recur (+ 2 v) (rest actions))))))

(defn solve [steps f]
  (apply + (for [x (range (inc 1000))
                 y (range (inc 1000))]
             (->> (applicable-steps steps x y)
                  (map first)
                  f))))


#_(solve [[:turn-on 0 0 999 999]])
#_(solve [[:toggle 0 0 999 0]])
#_(solve [[:toggle 0 0 999 999]
        [:toggle 499 499 500 500]])
